...
class postApi extends ApiController
{
    ...
    function register_routes()
    {
        register_rest_route("microsoft/v1", "/msn-article-retrieval", [
            ...
            'callback' => function ($data) {
                ...
                $publishStatus = array_map(function ($item) {
                    if (isset($item->id)) {
                        $post = MSPostMetaService::get_post_by_msn_id($item->id);
                        return [
                            'id' => $item->id,
                            'msn_status' => $item->status,
        ...
        register_rest_route("microsoft/v1", "/msn-article-detail/(?P<msnId>[A-Za-z0-9_]+)", [
            "methods" => "GET",
            'permission_callback' => function () {
                return current_user_can('edit_posts');
            },
            'callback' => function ($data) {
                $msn_id = $data['msnId'];
                $post = MSPostMetaService::get_post_by_msn_id($msn_id);
                if ($post) {
                    return array_merge(
                        ["id" => $msn_id],
                        MSPostConvertService::get_post_detail($post),
                        MSPostMetaService::get_all_ms_post_meta($post->ID)
                    );
                } else {
                    return null;
                }
        ...
        register_rest_route("microsoft/v1", "/post-detail/(?P<postId>[0-9_]+)", [
            ...
            'callback' => function ($data) {
                ...
                return array_merge(
                    ["id" => $post->ID],
                    MSPostConvertService::get_post_detail($post),
                    MSPostMetaService::get_default_ms_post_meta()
                );
            }
        ]);
            ...
            'callback' => function ($data) {
                ...
                for ($i = 0; $i < count($posts); $i++) {
                    ...
                    if ((($now - $start_time) < $max_life_time) && !Options::get_batch_submit_stop()) {
                        ...
                        LogService::add_log(LoggerTelemetryType::Log, LoggerFeatureSet::ContentOnboarding, "After set meta", array(
                            'ID' => $post->ID,
