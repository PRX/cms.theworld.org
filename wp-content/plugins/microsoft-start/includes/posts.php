...
?>
                    <div id=<?= 'msn-post-'.$post_ids ?> data-msn="post" data-post-id=<?= $post_ids ?> data-post-status=<?= $post->post_password ? 'password' : $post->post_status  ?>  data-post-is-empty=<?= !$post->post_content ?>></div>
<?php
                } else {
?>
                    <div id=<?= get_post_meta($post_ids, "msn_id", true) ?> data-msn="msn"></div>
<?php
                }
        }
    }
    ...
    static function sync_new_post_to_MSPH($post, $origin) {
        ...
        LogService::add_log(LoggerTelemetryType::Log, LoggerFeatureSet::ContentOnboarding, "After compose", array(
            'ID' => $post->ID,
            'msn_id' => $msPost['id'],
            'origin' => $origin
        ));
    ...
    function enqueue_script_in_post_list_page()
    {
        ...
        wp_localize_script(
            ...
            [
                ...
                'dataEnv' => MSPH_WORDPRESS_DEBUG === '1' ? 'int' : 'prod',
