...
class MSPostMetaService
{
    ...
    {
        static::update_meta_data_by_id($postID, static::get_default_ms_post_meta(true));
    }
    ...
    public static function update_meta_data_from_request($postID)
    {
        ...
        if (is_array($meta_from_request)) {
            static::update_meta_data_by_id($postID, $meta_from_request);
        }
    }
    ...
    {
        global $wpdb;
        $meta_row = $wpdb->get_row($wpdb->prepare("SELECT * FROM $wpdb->postmeta WHERE meta_key='msn_id' AND meta_value='$msn_id'"));
    ...
    public static function get_default_ms_post_meta()
    {
        ...
        return [
            ...
            "MSN_Has_Custom_Author" => "0",
            "MSN_Custom_Author" => "",
            "MSN_Has_Custom_Canonical_Url" => "0",
            "MSN_Custom_Canonical_Url" => ""
        ];
    }

    private static function update_meta_data_by_id($postID, $meta)
    {
        foreach (static::$ms_post_meta_keys as $key) {
    ...
    public static function unpublished_posts($page)
    {
        ...
        return array_map(function($post) {
            $post->post_feature_image = get_the_post_thumbnail_url($post);
            return $post;
        }, $posts) ;
    }
}
