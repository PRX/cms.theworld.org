...
class UrlToQueryItem {
	 ...
	 * @var array
	 */
	private $post_type_query_vars = array();
	 ...
	 */
	function resolve( $url = '', array $query_string_vars = array() ) {
	 ...
	 */
	private function parseUrl( $url = '', array $query_string_vars = array() ) {
		$url_query_string = parse_url( $url, PHP_URL_QUERY );
		if ( $url_query_string ) {
			parse_str( $url_query_string, $this->query_string );
		}
	 ...
	 */
	private function setMatchedQuery( $matches = array(), $query = '' ) {
		if ( ! is_null( $this->matched_rule ) ) {
	...
	private function setCptQueryVars() {
		foreach ( get_post_types( array(), 'objects' ) as $post_type => $t ) {
			if ( $t->query_var ) {
				$this->post_type_query_vars[ $t->query_var ] = $post_type;
			}
		}
	}
	 ...
	 */
	private function parseQueryVars( array $public_vars = array(), array $extra = array() ) {
	 ...
	 * @param array $private
	 */
	private function parsePrivateQueryVars( array $extra = array(), array $private = array() ) {
		if ( ! empty( $extra ) ) {
