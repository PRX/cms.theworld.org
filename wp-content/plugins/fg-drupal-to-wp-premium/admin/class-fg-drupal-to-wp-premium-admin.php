...
class FG_Drupal_to_WordPress_Premium_Admin extends FG_Drupal_to_WordPress_Admin {
	...
	public function process_admin_page( $data ) {
		...
		// DINKUM: Partial import taxonomies
		$data['partial_import_taxonomies'] = get_option( 'fgd2wp_partial_import_taxonomies_html' );
		...
		foreach ( $images_data as $index => $image_data ) {
			...
			if ( ! empty( $image ) ) {
				...
				if ( preg_match( '/^video/', $image['filemime'] ) ) {
					// DINKUM: Video support.
					if ( function_exists( 'pri_transform_attachment_url' ) ) {
						$filename = pri_transform_attachment_url( $image['uri'] );
					}
	...
	public function get_custom_field_values( $entity_id, $entity, $custom_field, $entity_type ) {
		...
		if ( isset( $custom_field['table_name'] ) && ! empty( $custom_field['columns'] ) ) {
			...
			// DINKUM: Add extra custom field support.
			$args_pmh_get_custom_field_values                        = apply_filters( 'pmh_get_custom_field_values', array( $entity_id, $entity, $custom_field, $entity_type ) );
			list( $entity_id, $entity, $custom_field, $entity_type ) = $args_pmh_get_custom_field_values;
			...
			if ( $this->table_exists( $table_name ) ) {
				...
				switch ( $field_type ) {
					...
					case 'embed':
						...
						/*
						 Debug
						echo "<pre>get_custom_field_values - file,media,embed";
						var_dump( $field_value_name );
						var_dump( $table_name );
						var_dump( $field_file_id_field );
						var_dump( $entity_id );
						var_dump( $sql );
						echo "</pre>";
						*/
