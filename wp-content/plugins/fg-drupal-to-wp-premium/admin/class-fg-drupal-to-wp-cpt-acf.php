...
if ( ! class_exists( 'FG_Drupal_to_WordPress_CPT_ACF', false ) ) {
	...
	class FG_Drupal_to_WordPress_CPT_ACF {
		...
		/**
		 * Register a builtin taxonomy on CPT UI
		 *
		 * @since 3.28.0
		 *
		...
		public function register_builtin_taxonomy( $taxonomy, $singular, $plural, $description, $post_types, $hierarchical ) {
			// Builtin taxonomies are not registered with CPT UI
		}
		...
		public function create_acf5_field( $field_slug, $field, $post_type, $fields_group_id ) {
			...
			// DINKUM: Intercept ACF field creation.
			$field_slug = apply_filters( 'pmh_create_acf_field_slug', $field_slug, $field, $post_type, $fields_group_id );
		...
		public function set_custom_post_field( $new_post_id, $custom_field_name, $custom_field, $custom_field_values, $date = '' ) {
			...
			// DINKUM: Custom post field process.
			do_action( 'pmh_set_custom_post_field', $new_post_id, $custom_field_name, $custom_field, $custom_field_values, $date, $this );

			$index = 0;
			// Repeater field
			$is_repeater = isset( $custom_field['repetitive'] ) && $custom_field['repetitive'] && $this->is_repeating_fields_supported();
			if ( $is_repeater ) {
				// DINKUM: add a filter to modify the field before adding the values.
				$repeater_field_name = apply_filters( 'pmh_set_collection_field_name', 'collection-' . $custom_field_name );
				$index               = intval( get_post_meta( $new_post_id, $repeater_field_name, true ) );
				$meta_key_like       = $repeater_field_name . '\_%\_' . $custom_field_name;
			...
			// DINKUM: Add support to array value fields.
			if ( isset( $custom_field['is_array'] ) && $custom_field['is_array'] ) {
			...
			foreach ( $meta_values as $meta_value ) {
				...
				// DINKUM: Setup featured image.
				if ( 'image' === $custom_field_name ) {
					set_post_thumbnail( $new_post_id, $meta_value );
				}
			}
		...
		public function set_custom_term_field( $new_term_id, $custom_field_name, $custom_field, $custom_field_values ) {
			// DINKUM: Custom post field process added to support repeater.
			$index = 0;
			// Repeater field
			$is_repeater = isset( $custom_field['repetitive'] ) && $custom_field['repetitive'] && $this->is_repeating_fields_supported();
			if ( $is_repeater ) {
				$repeater_field_name = 'collection-' . $custom_field_name;
				$index               = intval( get_term_meta( $new_term_id, $repeater_field_name, true ) );
				$meta_key_like       = $repeater_field_name . '\_%\_' . $custom_field_name;
				$current_meta_values = $this->plugin->get_term_meta_like( $new_term_id, $meta_key_like );
				$custom_field_name   = $repeater_field_name . '_%d_' . $custom_field_name;
			}
			...
			if ( $is_repeater ) {
				// Update the last index of the repeater field
				update_term_meta( $new_term_id, $repeater_field_name, $index );
				if ( isset( $this->custom_fields[ $repeater_field_name ] ) ) {
					update_term_meta( $new_term_id, '_' . $repeater_field_name, $this->custom_fields[ $repeater_field_name ] );
				}
			}
		}
					...
					if ( ! $this->plugin->plugin_options['skip_media'] ) {
						foreach ( $custom_field_values as $file ) {
							...
							$image_attributs = array(
								...
								'media_credit'  => isset( $file['credit'] ) ? $file['credit'] : '',
								'hide_image'    => isset( $file['hide_image'] ) ? $file['hide_image'] : '0',
							);

							// DINKUM: Add file metas.
							$image_attributs = apply_filters( 'pmh_image_attributes', $image_attributs, $file );
							...
							/*
							 Debug
							echo "<pre>";
							var_dump( 'convert_custom_field_to_meta_values' );
					...
					if ( ! $this->plugin->plugin_options['skip_media'] ) {
						foreach ( $custom_field_values as $file ) {
							...
							if ( $attachment_id ) {
								...
								// Assign the media URL to the postmeta
								if ( ! empty( $new_post_id ) ) {
									$this->plugin->add_post_media( $new_post_id, array( 'post_date' => $file_date ), array( $attachment_id ), false ); // Attach the media to the post
								}
								// Set the field value
								$meta_values[] = $attachment_id;
							}
