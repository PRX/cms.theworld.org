	...
	class FG_Drupal_to_WordPress_Custom_Content {
		...
		public function get_total_elements_count( $count ) {
			...
			if ( ! isset( $this->plugin->premium_options['skip_taxonomies'] ) || ! $this->plugin->premium_options['skip_taxonomies'] ) {
				$taxonomies = $this->get_custom_taxonomies();
				foreach ( $taxonomies as $taxonomy ) {
					$tax_type_type = $taxonomy['machine_name'];
					if ( ! isset( $this->plugin->premium_options['taxonomies_to_skip'] ) || ! in_array( $tax_type_type, $this->plugin->premium_options['taxonomies_to_skip'] ) ) {
		...
		private function get_custom_taxonomies() {
			...
			// DINKUM: added to exclude some taxonomies to be processed.
			return apply_filters( 'fgd2wp_get_custom_taxonomies', $taxonomies );
		}

		/**
		 * DINKUM: Save the partial import taxonomies options
		 */
		public function save_partial_import_taxonomies_options() {
			if ( $this->plugin->drupal_connect() ) {
				$this->get_partial_import_taxonomies_content();
			}
		}

		/**
		 * DINKUM: Get the partial import taxonomies content
		 *
		 * @return string HTML content
		 */
		public function get_partial_import_taxonomies_content() {
			global $drupal_db;

			$content = '';
			if ( ! empty( $drupal_db ) ) {
				$taxonomy_types = $this->get_custom_taxonomies();
				...
				if ( count( $taxonomy_types ) > 0 ) {
					$tax_to_skip = isset( $this->plugin->premium_options['taxonomies_to_skip'] ) ? $this->plugin->premium_options['taxonomies_to_skip'] : array();
					if ( empty( $tax_to_skip ) ) {
						$tax_to_skip = array();
					}
					foreach ( $taxonomy_types as $tax_type ) {
						$tax_type_type = $tax_type['machine_name'];
						$skip_tax      = in_array( $tax_type_type, $tax_to_skip );
						$content      .= '<input id="skip_tax_' . $tax_type_type . '" name="taxonomies_to_skip[]" type="checkbox" value="' . $tax_type_type . '" ' . checked( $skip_tax, 1, false ) . ' /> <label for="skip_tax_' . $tax_type_type . '" >' . sprintf( __( "Don't import taxonomy %1\$s (%2\$s)", 'fgd2wpp' ), $tax_type['name'], $tax_type_type ) . "</label><br />\n";
					}
				}
				update_option( 'fgd2wp_partial_import_taxonomies_html', $content );
			}
			return $content;
		}
			...
			if ( ! empty( $drupal_db ) ) {
				...
				if ( count( $node_types ) > 0 ) {
					...
					foreach ( $node_types as $node_type ) {
						...
						$content       .= '<input id="skip_node_' . $node_type_type . '" name="nodes_to_skip[]" type="checkbox" value="' . $node_type_type . '" ' . checked( $skip_node, 1, false ) . ' /> <label for="skip_node_' . $node_type_type . '" >' . sprintf( __( "Don't import the nodes of type %1\$s (%2\$s)", 'fgd2wpp' ), $node_type['name'], $node_type_type ) . "</label><br />\n";
					}
				}
		...
		public function register_custom_content() {
			...
			if ( ! isset( $this->plugin->premium_options['skip_taxonomies'] ) || ! $this->plugin->premium_options['skip_taxonomies']
				...
				// DINKUM: Debug node types and relationships
				if ( defined( 'WP_CLI' ) ) {
					WP_CLI::log( 'Nodes Types for Taxonomies Relations: ' . json_encode( $this->node_types_for_taxonomy ) );
					WP_CLI::log( 'Nodes Taxonomies for Node Type Relations: ' . json_encode( $this->taxonomies_for_node_type ) );
				}
				// if ( defined( 'WP_CLI' ) ) {
				// WP_CLI::log( 'Nodes Type Relations: ' . json_encode( $this->node_types_relations ) );
				// }
				...
				// DINKUM: Debug node types and custom fields
				// if ( defined( 'WP_CLI' ) ) {
				// WP_CLI::log( 'Taxonomies Custom Fields: ' . json_encode( $this->taxonomy_custom_fields ) );
				// }
				$this->register_custom_fields();
				// DINKUM: Debug node types and custom fields
				// if ( defined( 'WP_CLI' ) ) {
				// WP_CLI::log( 'Nodes Custom Fields: ' . json_encode( $this->custom_fields ) );
				// }
			...
			foreach ( $raw_custom_fields as $row ) {
				...
				if ( isset( $data['field_type'] ) ) {
					if ( isset( $data['label'] ) ) {
						...
						if ( in_array( $data['field_type'], array( 'text', 'image', 'list', 'options', 'datetime', 'text_long', 'text_with_summary', 'textstyled', 'list_string', 'integer', 'float', 'decimal', 'string', 'string_long', 'email', 'telephone', 'link', 'file', 'boolean', 'video_embed_field', 'color_field_type' ) ) ) {
		...
		public function set_node_taxonomies_relations( $new_post_id, $node, $post_type = 'post', $entity_type = 'node' ) {
			...
			foreach ( $terms_by_taxonomy as $taxonomy => $terms ) {
				...
				// Dinkum: Avoid uncategorized for category taxonomy.
				if ( 'category' === $taxonomy ) {

					wp_set_object_terms( $new_post_id, $terms_ids, $taxonomy_slug );
		...
		public function import_custom_taxonomies_terms() {
			...
			if ( ! isset( $this->plugin->premium_options['skip_taxonomies'] ) || ! $this->plugin->premium_options['skip_taxonomies'] ) {
				...
				foreach ( $taxonomies as $taxonomy ) {
					...
					if ( empty( $this->plugin->premium_options['taxonomies_to_skip'] ) || ( is_array( $this->plugin->premium_options['taxonomies_to_skip'] ) && ! in_array( $taxonomy['machine_name'], $this->plugin->premium_options['taxonomies_to_skip'] ) ) ) {
						$terms_count = $this->plugin->import_taxonomies_terms( $taxonomy['machine_name'], $taxonomy['name'] );
						$this->plugin->display_admin_notice( sprintf( __( '%1$d %2$s imported', 'fgd2wpp' ), $terms_count, $taxonomy['name'] ) );
					}
				}
			}
		}
			...
			if ( ! isset( $this->plugin->premium_options['skip_nodes'] ) || ! $this->plugin->premium_options['skip_nodes'] ) {
				...
				foreach ( $nodes_types as $node_type ) {
					...
					if ( ! isset( $this->plugin->premium_options['nodes_to_skip'] ) || ! in_array( $node_type_type, $this->plugin->premium_options['nodes_to_skip'] ) ) {
						...
						$this->plugin->display_admin_notice( sprintf( __( '%1$d %2$s imported', 'fgd2wpp' ), $nodes_count, FG_Drupal_to_WordPress_Tools::plural( $node_type['name'] ) ) );
					}
				}
			}
		}
		...
		public function import_node_fields( $new_post_id, $node, $post_type = 'post', $entity_type = 'node' ) {
			if ( isset( $this->custom_fields[ $entity_type ][ $node['type'] ] ) ) {
				...
				foreach ( $this->custom_fields[ $entity_type ][ $node['type'] ] as $custom_field_name => $custom_field ) {
					...
					// DINKUM: Add a way to exclude the import field before it executes the database query.
					$import_custom_field = apply_filters( "fgd2wp_import_{$node['type']}_field", true, $custom_field_name );
					if ( $this->is_node_reference_field( $custom_field ) || ! $import_custom_field ) {
					...
					/*
					 Debug
					if ( 'video' === $custom_field_name ) {
						echo '<pre>import_node_fields - custom_field_values';
					...
					if ( ! empty( $custom_field_values ) ) {
						...
						switch ( $custom_field_name ) {
							...
							case 'excerpt':
								...
								break;
							case 'format':
								if ( in_array( $custom_field_values, array( 'standard', 'aside' ), true ) ) {
									$this->update_custom_field_as_post_field( $new_post_id, 'post_format', $custom_field_values, $date );
								}
		...
		/**
		 * DINKUM: added to support post format field update
		 ...
		 * @param date   $date Node date
		 */
		private function update_custom_field_as_post_field( $new_post_id, $field, $field_values, $date = '' ) {
			if ( ! empty( $field_values ) ) {
				if ( is_array( $field_values[0] ) ) {
					$value = implode( "<br />\n", $field_values[0] );
				} else {
		...
		public function import_term_custom_fields( $new_term_id, $term, $wp_taxonomy ) {
			if ( isset( $this->taxonomy_custom_fields[ $wp_taxonomy ] ) ) {
				foreach ( $this->taxonomy_custom_fields[ $wp_taxonomy ] as $custom_field_name => $custom_field ) {
					// DINKUM: Add a way to exclude the import field before it executes the database query.
					$import_custom_field = apply_filters( "fgd2wp_import_term_{$wp_taxonomy}_field", true, $custom_field_name );
					if ( ! $import_custom_field ) {
						continue; // Don't import the the field.
					}
