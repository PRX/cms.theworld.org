jQuery(document).ready(function ($) {
  const fGetObjSettings = function () {
    return {
      iPaged: parseInt(
        $('#pmh-media-fix-form [name="pmh-media-fix-paged"]').val()
      ),
      iPerPage: parseInt(
        $('#pmh-media-fix-form [name="pmh-media-fix-perpage"]').val()
      ),
  ...
  $("#pmh-media-fix-form").on("process-start", function (e) {
    ...
    if ("" != ObjSettings.sPerIds) {
      $('#pmh-media-fix-form [name="pmh-media-fix-ids"]').attr(
        "readonly",
        true
      );
    }
  ...
  $("#pmh-media-fix-form").on("process-media-finished", function (e) {
    ...
    if ("" === ObjSettings.sPerIds) {
      ...
      $('#pmh-media-fix-form [name="pmh-media-fix-paged"]').val(1);
    }
  });
  ...
  $("#pmh-post-worker-posts-fix").on("click", function (e) {
    ...
    fRunPostsFix(function (response) {
      if (response.count_processed === 0) {
        $("#pmh-media-fix-form").trigger("process-post-finished");
      }
